...
def test_extract_datasets_handles_product_number_headers() -> None:
    workbook = Workbook()
    sheet = workbook.active
    sheet.title = "Item Catalog"
    sheet.append(
        [
            "Product Number",
            "Description",
    ...
    sheet.append(
        [
            ...
            "Aisle 4-B",
            12,
            "Preferred widgets vendor",
        ]
    )

    buffer = _save_workbook(workbook)
    datasets = extract_datasets(
        buffer.getvalue(), "products.xlsx", preferred_entity="products"
    )

    assert "products" in datasets
    assert len(datasets["products"]) == 1
    product = datasets["products"][0]
    assert product["sku"] == "ITEM-001"
    assert product["description"] == "Ultra Widget"
    assert product["vendor_name"] == "Widget Supply"
    assert product["category"] == "BrightCo"
    assert product["qty_on_hand"] == 12
    assert product["location_name"] == "Aisle 4-B"
