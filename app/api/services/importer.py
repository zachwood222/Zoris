...
async def import_spreadsheet(
    ...
    items_index = await _import_products(
        session, datasets.get("products", []), counters, vendor_index, location_index
    )
    customers_index = await _import_customers(session, datasets.get("customers", []), counters)
...
async def _import_customers(
    ...
    for row in rows:
        ...
        if any(candidate in customers for candidate in key_candidates):
            continue
